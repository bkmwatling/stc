...
#define stc_vec_new(T, cap)                                         \
    __extension__({                                                 \
        ...
        (StcVec(T)){ malloc(sizeof(_STC_FATP_MAGIC_TYPE_MACRO(T)) * \
                     ...
                     0, _STC_MACRO_VAR(_stc_vec_new_cap_) };        \
    })

/**
 * Initialise a vector's memory to a given capacity, setting the length to 0.
 *
 * @param[in,out] v   a pointer to the vector to initialise
 * @param[in]     cap the capacity to the initialise the vector with
 */
#define stc_vec_init(v, cap)                                                   \
    __extension__({                                                            \
        __auto_type _STC_MACRO_VAR(_stc_vec_init_v_) = (v);                    \
        _STC_MACRO_VAR(_stc_vec_init_v_)->len        = 0;                      \
        _STC_MACRO_VAR(_stc_vec_init_v_)->cap        = (cap);                  \
        _STC_MACRO_VAR(_stc_vec_init_v_)->__stc_slice_data =                   \
            malloc(sizeof(*_STC_MACRO_VAR(_stc_vec_init_v_)->__stc_vec_data) * \
                   _STC_MACRO_VAR(_stc_vec_init_v_)->cap);                     \
    })
 ...
 * length to 0.
 *
 * @param[in,out] v a pointer to the vector to initialise with default capacity
 */
...
#define stc_vec_clone(v)                                             \
    __extension__({                                                  \
        ...
        __auto_type _STC_MACRO_VAR(_stc_vec_clone_data_) =           \
            _STC_MACRO_VAR(_stc_vec_clone_v_).__stc_vec_data;        \
        _STC_MACRO_VAR(_stc_vec_clone_v_).__stc_vec_data =           \
            malloc(sizeof(*_STC_MACRO_VAR(_stc_vec_clone_data_)) *   \
                   _STC_MACRO_VAR(_stc_vec_clone_v_).len);           \
        _STC_MACRO_VAR(_stc_vec_clone_v_).cap =                      \
            _STC_MACRO_VAR(_stc_vec_clone_v_).len;                   \
        if (_STC_MACRO_VAR(_stc_vec_clone_v_).len)                   \
            memcpy(_STC_MACRO_VAR(_stc_vec_clone_v_).__stc_vec_data, \
                   _STC_MACRO_VAR(_stc_vec_clone_data_),             \
                   sizeof(*_STC_MACRO_VAR(_stc_vec_clone_data_)) *   \
                       _STC_MACRO_VAR(_stc_vec_clone_v_).len);       \
 ...
 *
 * @param[in,out] v a pointer to the vector to add the element to
 * @param[in]     x the element to add to the vector
 */
#define stc_vec_push_back(v, x)                                         \
    __extension__({                                                     \
        __auto_type _STC_MACRO_VAR(_stc_vec_push_back_v_) = (v);        \
        stc_vec_reserve(_STC_MACRO_VAR(_stc_vec_push_back_v_), 1);      \
        stc_vec_at(*_STC_MACRO_VAR(_stc_vec_push_back_v_),              \
                   _STC_MACRO_VAR(_stc_vec_push_back_v_)->len++) = (x); \
    })
 ...
 *
 * @param[in,out] v a pointer to the vector to pop the tail element from
 *
 * @return the tail element of the vector
 */
#define stc_vec_pop_back(v)                                      \
    __extension__({                                              \
        __auto_type _STC_MACRO_VAR(_stc_vec_pop_back_v_) = (v);  \
        stc_vec_at(*_STC_MACRO_VAR(_stc_vec_pop_back_v_),        \
                   --_STC_MACRO_VAR(_stc_vec_pop_back_v_)->len); \
    })

/**
 * Remove and return the element at the head of a vector.
 *
 * @param[in,out] v a pointer to the vector to pop the head element from
 *
 * @return the head element of the vector
 */
#define stc_vec_pop_front(v)                                                 \
    __extension__({                                                          \
        __auto_type _STC_MACRO_VAR(_stc_vec_pop_front_v_) = (v);             \
        __auto_type _STC_MACRO_VAR(_stc_vec_pop_front_x_) =                  \
            stc_vec_at(*_STC_MACRO_VAR(_stc_vec_pop_front_v_), 0);           \
        _stc_vec_shift(                                                      \
            _STC_VEC_PTR_ANY(_STC_MACRO_VAR(_stc_vec_pop_front_v_)), 1, 0,   \
            sizeof(*_STC_MACRO_VAR(_stc_vec_pop_front_v_)->__stc_vec_data)); \
        _STC_MACRO_VAR(_stc_vec_pop_front_v_)->len--;                        \
...
#define stc_vec_last(v)                                       \
    __extension__({                                           \
        ...
        stc_vec_at(_STC_MACRO_VAR(_stc_vec_last_v_),          \
                   _STC_MACRO_VAR(_stc_vec_last_v_).len - 1); \
    })

/**
 * Insert an element into a vector at the specified index.
 *
 * @param[in,out] v a pointer to the vector to insert the element into
...
#define stc_vec_insert(v, i, x)                                       \
    __extension__({                                                   \
        __auto_type _STC_MACRO_VAR(_stc_vec_insert_v_) = (v);         \
        __auto_type _STC_MACRO_VAR(_stc_vec_insert_i_) = (i);         \
        stc_vec_reserve_index(_STC_MACRO_VAR(_stc_vec_insert_v_),     \
                              _STC_MACRO_VAR(_stc_vec_insert_i_), 1); \
        _STC_MACRO_VAR(_stc_vec_insert_v_)->len++;                    \
        stc_vec_at(*_STC_MACRO_VAR(_stc_vec_insert_v_),               \
                   _STC_MACRO_VAR(_stc_vec_insert_i_)) = (x);         \
    })

/**
 * Remove and return the element at the specified index from a vector.
 *
 * @param[in,out] v a pointer to the vector to remove the element from
...
#define stc_vec_remove(v, i)                                  \
    __extension__({                                           \
        __auto_type _STC_MACRO_VAR(_stc_vec_remove_v_) = (v); \
        __auto_type _STC_MACRO_VAR(_stc_vec_remove_i_) = (i); \
        __auto_type _STC_MACRO_VAR(_stc_vec_remove_x_) =      \
            stc_vec_at(*_STC_MACRO_VAR(_stc_vec_remove_v_),   \
                       _STC_MACRO_VAR(_stc_vec_remove_i_));   \
        stc_vec_drain(_STC_MACRO_VAR(_stc_vec_remove_v_),     \
...
#define stc_vec_swap_remove(v, i)                                      \
    __extension__({                                                    \
        __auto_type _STC_MACRO_VAR(_stc_vec_swap_remove_v_) = (v);     \
        __auto_type _STC_MACRO_VAR(_stc_vec_swap_remove_i_) = (i);     \
        __auto_type _STC_MACRO_VAR(_stc_vec_swap_remove_x_) =          \
            stc_vec_at(*_STC_MACRO_VAR(_stc_vec_swap_remove_v_),       \
                       _STC_MACRO_VAR(_stc_vec_swap_remove_i_));       \
        stc_vec_at(*_STC_MACRO_VAR(_stc_vec_swap_remove_v_),           \
                   _STC_MACRO_VAR(_stc_vec_swap_remove_i_)) =          \
            stc_vec_pop_back(_STC_MACRO_VAR(_stc_vec_swap_remove_v_)); \
 ...
 *       up memory is left to the caller.
 *
 * @param[in,out] v   a pointer to the vector to truncate
...
#define stc_vec_truncate(v, len)                                    \
    __extension__({                                                 \
        __auto_type _STC_MACRO_VAR(_stc_vec_truncate_v_)   = (v);   \
        ...
        0 <= _STC_MACRO_VAR(_stc_vec_truncate_len_) &&              \
                _STC_MACRO_VAR(_stc_vec_truncate_len_) <            \
                    _STC_MACRO_VAR(_stc_vec_truncate_v_)->len       \
            ? (_STC_MACRO_VAR(_stc_vec_truncate_v_)->len =          \
                   _STC_MACRO_VAR(_stc_vec_truncate_len_),          \
               1)                                                   \
            : 0;                                                    \
 ...
 *
 * @param[in,out] v a pointer to the vector to move elements to the end of
 * @param[in,out] w the vector to move the elements from
 */
#define stc_vec_append(v, w)                                                  \
    __extension__({                                                           \
        __auto_type _STC_MACRO_VAR(_stc_vec_append_w_) = (w);                 \
        stc_vec_extend(v, _STC_MACRO_VAR(_stc_vec_append_w_)->__stc_vec_data, \
                       _STC_MACRO_VAR(_stc_vec_append_w_)->len);              \
        stc_vec_clear(_STC_MACRO_VAR(_stc_vec_append_w_));                    \
    })
 ...
 * specified length.
 *
 * @param[in,out] v   a pointer to the vector to extend
...
#define stc_vec_extend(v, p, len)                                             \
    __extension__({                                                           \
        __auto_type _STC_MACRO_VAR(_stc_vec_extend_v_) = (v);                 \
        __auto_type _STC_MACRO_VAR(_stc_vec_extend_p_) = (p);                 \
        STC_TYPECHECK(                                                        \
            __typeof__(_STC_MACRO_VAR(_stc_vec_extend_v_)->__stc_vec_data),   \
            _STC_MACRO_VAR(_stc_vec_extend_p_));                              \
        _stc_vec_extend(_STC_VEC_PTR_ANY(_STC_MACRO_VAR(_stc_vec_extend_v_)), \
                        _STC_MACRO_VAR(_stc_vec_extend_p_),                   \
...
#define stc_vec_extend_from_slice(v, s)                                        \
    __extension__({                                                            \
        ...
        stc_vec_extend(                                                        \
            v, _STC_MACRO_VAR(_stc_vec_extend_from_slice_s_).__stc_slice_data, \
            _STC_MACRO_VAR(_stc_vec_extend_from_slice_s_).len);                \
    })
 ...
 *
 * @param[in,out] v a pointer to the vector to reserve space for
 * @param[in]     n the number of elements to reserve space for in the vector
 */
#define stc_vec_reserve(v, n)                                             \
    __extension__({                                                       \
        __auto_type _STC_MACRO_VAR(_stc_vec_reserve_v_) = (v);            \
        _stc_vec_reserve(                                                 \
            _STC_VEC_PTR_ANY(_STC_MACRO_VAR(_stc_vec_reserve_v_)),        \
            sizeof(*_STC_MACRO_VAR(_stc_vec_reserve_v_)->__stc_vec_data), \
            (n));                                                         \
    })
...
#define stc_vec_reserve_exact(v, n)                                          \
    __extension__({                                                          \
        __auto_type _STC_MACRO_VAR(_stc_vec_reserve_exact_v_) = (v);         \
        _stc_vec_reserve_exact(                                              \
            _STC_VEC_PTR_ANY(_STC_MACRO_VAR(_stc_vec_reserve_exact_v_)),     \
            sizeof(                                                          \
                *_STC_MACRO_VAR(_stc_vec_reserve_exact_v_)->__stc_vec_data), \
            (n));                                                            \
    })
...
#define stc_vec_reserve_index(v, i, n)                                        \
    __extension__({                                                           \
        __auto_type _STC_MACRO_VAR(_stc_vec_reserve_index_v_) = (v);          \
        __auto_type _STC_MACRO_VAR(_stc_vec_reserve_index_i_) = (i);          \
        __auto_type _STC_MACRO_VAR(_stc_vec_reserve_index_n_) = (n);          \
        stc_vec_reserve(_STC_MACRO_VAR(_stc_vec_reserve_index_v_),            \
                        _STC_MACRO_VAR(_stc_vec_reserve_index_n_));           \
        _stc_vec_shift(                                                       \
            _STC_VEC_PTR_ANY(_STC_MACRO_VAR(_stc_vec_reserve_index_v_)),      \
            _STC_MACRO_VAR(_stc_vec_reserve_index_i_),                        \
            _STC_MACRO_VAR(_stc_vec_reserve_index_i_) +                       \
                _STC_MACRO_VAR(_stc_vec_reserve_index_n_),                    \
            sizeof(                                                           \
                *_STC_MACRO_VAR(_stc_vec_reserve_index_v_)->__stc_vec_data)); \
    })
 ...
 *
 * @param[in,out] v   a pointer to the vector to shrink
 * @param[in]     cap the capacity to shrink the vector to
 */
#define stc_vec_shrink(v, cap)                                           \
    __extension__({                                                      \
        __auto_type _STC_MACRO_VAR(_stc_vec_shrink_v_) = (v);            \
        _stc_vec_shrink(                                                 \
            _STC_VEC_PTR_ANY(_STC_MACRO_VAR(_stc_vec_shrink_v_)),        \
            sizeof(*_STC_MACRO_VAR(_stc_vec_shrink_v_)->__stc_vec_data), \
            (cap));                                                      \
    })
 ...
 *
 * @param[in,out] v a pointer to the vector to shrink to its length
 */
#define stc_vec_shrink_to_fit(v)                                        \
    __extension__({                                                     \
        __auto_type _STC_MACRO_VAR(_stc_vec_shrink_to_fit_v_) = (v);    \
        stc_vec_shrink(_STC_MACRO_VAR(_stc_vec_shrink_to_fit_v_),       \
                       _STC_MACRO_VAR(_stc_vec_shrink_to_fit_v_)->len); \
    })
...
#define stc_vec_as_slice(v)                                                   \
    __extension__({                                                           \
        __auto_type    _STC_MACRO_VAR(_stc_vec_as_slice_v_) = (v);            \
        StcSlice(void) _STC_MACRO_VAR(_stc_vec_as_slice_s_) = {               \
            _STC_MACRO_VAR(_stc_vec_as_slice_v_).__stc_vec_data,              \
            _STC_MACRO_VAR(_stc_vec_as_slice_v_).len                          \
        };                                                                    \
        _STC_FATP_AUTO_CAST_VAL_TO_VAL(StcVec, StcSlice,                      \
                                       _STC_MACRO_VAR(_stc_vec_as_slice_v_),  \
                                       _STC_MACRO_VAR(_stc_vec_as_slice_s_)); \
    })
...
#define stc_vec_to_slice(v)                                                    \
    __extension__({                                                            \
        ...
        StcSlice(void) _STC_MACRO_VAR(_stc_vec_to_slice_s_) = {                \
            malloc(                                                            \
                sizeof(*_STC_MACRO_VAR(_stc_vec_to_slice_v_).__stc_vec_data) * \
                _STC_MACRO_VAR(_stc_vec_to_slice_v_).len),                     \
            _STC_MACRO_VAR(_stc_vec_to_slice_v_).len                           \
        };                                                                     \
        if (_STC_MACRO_VAR(_stc_vec_to_slice_v_).len)                          \
            memcpy(                                                            \
                _STC_MACRO_VAR(_stc_vec_to_slice_s_).__stc_slice_data,         \
                _STC_MACRO_VAR(_stc_vec_to_slice_v_).__stc_vec_data,           \
                sizeof(*_STC_MACRO_VAR(_stc_vec_to_slice_v_).__stc_vec_data) * \
                    _STC_MACRO_VAR(_stc_vec_to_slice_v_).len);                 \
        _STC_FATP_AUTO_CAST_VAL_TO_VAL(StcVec, StcSlice,                       \
                                       _STC_MACRO_VAR(_stc_vec_to_slice_v_),   \
                                       _STC_MACRO_VAR(_stc_vec_to_slice_s_));  \
    })
